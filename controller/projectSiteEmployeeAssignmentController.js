...
class ProjectSiteEmployeeAssignmentController {
  createProjectSiteEmployeeAssignment = async (req, res) => {
    try {
      const { project_id, site_id, site_in_charge_id, active_status } =
        req.body;
      ...
      if (!project_id || !site_id || !site_in_charge_id) {
        return res.status(400).json({
          success: false,
          message:
            "Required fields missing: project_id, site_id, site_in_charge_id",
        });
      }
      ...
      const assignment_id = await insertData(
        "tx_project_site_employee_assignment",
        data,
      );
      ...
      console.error(
        "Error in createProjectSiteEmployeeAssignment:",
        error.message,
      );
      res.status(500).json({
        success: false,
  ...
  getAllProjectSiteEmployeeAssignments = async (req, res) => {
    try {
      ...
      console.error(
        "Error in getAllProjectSiteEmployeeAssignments:",
        error.message,
      );
      res.status(500).json({
        success: false,
  ...
  getProjectSiteEmployeeAssignmentById = async (req, res) => {
    try {
      ...
      console.error(
        "Error in getProjectSiteEmployeeAssignmentById:",
        error.message,
      );
      res.status(500).json({
        success: false,
  ...
  updateProjectSiteEmployeeAssignment = async (req, res) => {
    try {
      ...
      if (active_status !== undefined)
        updateValues.active_status = active_status;
      ...
      const affectedRows = await updateData(
        "tx_project_site_employee_assignment",
        updateValues,
        `project_site_employee_assignment_id = ${id}`,
      );
      if (affectedRows === 0) {
        return res
          .status(404)
      ...
      console.error(
        "Error in updateProjectSiteEmployeeAssignment:",
        error.message,
      );
      res.status(500).json({
        success: false,
  ...
  deleteProjectSiteEmployeeAssignment = async (req, res) => {
    try {
      ...
      const deletedRows = await deleteData(
        "tx_project_site_employee_assignment",
        `project_site_employee_assignment_id = ${id}`,
      );
      if (deletedRows === 0) {
      ...
      console.error(
        "Error in deleteProjectSiteEmployeeAssignment:",
        error.message,
      );
      res.status(500).json({
        success: false,
  ...
  getSiteInChargeBySiteId = async (req, res) => {
    try {
      const { site_id } = req.params;
      ...
      const table = `
      tx_project_site_employee_assignment AS a
      JOIN em_employees AS e 
      ON a.site_in_charge_id = e.employee_id
    `;
    ...
    `;
      ...
      const employee = await selectData(table, select, condition, null);
      ...
      console.error("Error in getSiteInChargeBySiteId:", error.message);

      res.status(500).json({
        success: false,
        message: "Unable to fetch site in charge details",
        error: error.message,
      });
    }
  };
