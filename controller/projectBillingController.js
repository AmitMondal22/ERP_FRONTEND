...
class projectBillingController {
  ...
  // };
   ...
   /* ============================================================
     1. CREATE BILLING ITEM
        — now takes project_work_description_id instead of
          project_work_description (free text)
     ============================================================ */
  createBillingItem = async (req, res) => {
    try {
      const {
        project_id,
        project_work_description_id,
        hsn_code,
        unit,
        quantity,
        rate,
      ...
      /* -------------------- Validation -------------------- */
      if (!project_id || !project_work_description_id) {
        return res.status(400).json({
          success: false,
          error: {
            code: "VALIDATION_ERROR",
            message: "project_id and project_work_description_id are required",
            fields: {
              project_id: !project_id ? "Required" : null,
              project_work_description_id: !project_work_description_id ? "Required" : null,
            },
          },
        });
      }

      if (isNaN(Number(project_work_description_id))) {
        return res.status(400).json({
          success: false,
          error: {
            code: "INVALID_WORK_DESCRIPTION",
            message: "project_work_description_id must be a valid id",
          },
        });
      }
      ...
      /* ---- Validate the work description id actually exists ---- */
      const wdCheck = await customSelectSqlQuery(
        `SELECT project_work_description_id FROM md_project_work_description WHERE project_work_description_id = ${Number(project_work_description_id)} LIMIT 1`,
        false
      );
      ...
      if (!wdCheck) {
        return res.status(400).json({
          ...
          error: {
            code: "INVALID_WORK_DESCRIPTION",
            message: "project_work_description_id does not exist",
          },
        });
      }
      ...
      const insertObj = {
        project_id,
        project_work_description_id: Number(project_work_description_id),
        hsn_code:     hsn_code     || null,
        unit,    
        quantity,
        rate,
...
/* ============================================================
     2. GET ALL BILLING ITEMS BY PROJECT
        — response shape UNCHANGED, project_work_description now
          sourced via JOIN with md_project_work_description
     ============================================================ */
    ...
    try {
      ...
      const sql = `
        SELECT
          pb.billing_id,
          pb.project_id,
          pb.project_work_description_id,
  ...
  updateBillingItem = async (req, res) => {
    try {
      ...
      if (!project_id || !project_work_description_id) {
        return res.status(400).json({
          success: false,
          message: "project_id & project_work_description_id are required",
        });
      }
