...
class StorController {
  ...
  // Add 
    createStore = async (req, res) => {
        try {
            ...
            if (!project_id || !store_name || !store_address || !store_incharge_id || !project_site_id) {
            return res.status(400).json({
                success: false,
                message: "Required fields missing: project_id, store_name, store_address, store_incharge_id"
            });
            }
  ...
  try {
    const table = `
      ...
      JOIN md_project_site AS ps ON s.project_site_id = ps.project_site_id
      LEFT JOIN em_employees AS m ON s.store_incharge_id = m.employee_id
    `;
...
updateStore = async (req, res) => {
  try {
    ...
    if (project_site_id) updateValues.project_site_id = project_site_id;
    if (store_name) updateValues.store_name = store_name;
    if (store_address) updateValues.store_address = store_address;
    if (store_incharge_id) updateValues.store_incharge_id = store_incharge_id;
