...
deletePurchase = async (req, res) => {
  ...
  try {
    if (!id || isNaN(Number(id))) {
      return res.status(400).json({
        success: false,
        message: "Valid purchase_id is required",
      });
    }
    ...
    // ----- LOCK & FETCH THE PURCHASE ROW -----
    const [purchaseRows] = await connection.execute(
      `SELECT purchase_id, project_id, site_id, stor_id
       FROM td_purchase
       WHERE purchase_id = ?
       FOR UPDATE`,
      [id]
    ...
    const { project_id, site_id, stor_id } = purchaseRows[0];
    console.log(
      `[DELETE PURCHASE] id=${id} project_id=${project_id} site_id=${site_id} stor_id=${stor_id}`
    );

    // ----- FETCH PRODUCTS TO REVERSE STOCK -----
    const [products] = await connection.execute(
      `SELECT product_id, invoice_qty
       FROM td_purchase_product
       WHERE purchase_id = ?`,
      [id]
    );

    console.log(`[DELETE PURCHASE] products to reverse:`, products);

    // ----- REVERSE STOCK PER PRODUCT (row-locked, clamped at 0) -----
    const stockWarnings = [];

    for (const p of products) {
      const [result] = await connection.execute(
        `UPDATE tx_current_stock
         SET invoice_qty = GREATEST(invoice_qty - ?, 0),
             updated_at = NOW()
         WHERE project_id = ? AND site_id = ? AND product_id = ? AND store_id = ?`,
        [p.invoice_qty ?? 0, project_id, site_id, p.product_id, stor_id]
      );

      // console.log(
      //   `[STOCK REVERSE] product_id=${p.product_id} qty=${p.invoice_qty} ` +
      //   `project=${project_id} site=${site_id} store=${stor_id} ` +
      //   `→ affectedRows=${result.affectedRows}`
      // );

      // No matching stock row found — flag it, don't silently continue
    ...
    await connection.execute(
      `DELETE FROM td_purchase_product WHERE purchase_id = ?`,
      [id]
    );

    // ----- DELETE PARENT ROW -----
    await connection.execute(
      `DELETE FROM td_purchase WHERE purchase_id = ?`,
      [id]
    );
    ...
    if (stockWarnings.length > 0) {
      console.warn(
        `[DELETE PURCHASE] purchase_id=${id} deleted, but no matching tx_current_stock row for:`,
        stockWarnings
      );
    }
    ...
    return res.status(200).json({
      ...
      // Surfaces silently-unmatched stock rows to the client/caller for visibility.
      // Remove this field once you've root-caused the mismatch, if you don't want
      // it in the permanent API contract.
      stock_reversal_warnings: stockWarnings.length ? stockWarnings : undefined,
    });
